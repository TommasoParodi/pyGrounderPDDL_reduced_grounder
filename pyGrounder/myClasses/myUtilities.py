from antlr4 import *
from pyGrounder.myClasses.antlr4_directory.pddlLexer import pddlLexer
from pyGrounder.myClasses.antlr4_directory.pddlParser import pddlParser
...
def remove_comments(file_path):
    with open(file_path, "r") as f:
        # List of lines
        lines = f.readlines()
    # list of lines without comments
    new_lines = []
    for line in lines:
        if line.startswith(";"):
            # skip the line if the comment appears at the beginning
            continue
        elif ";" in line:
            # remove the comment if it appears after the code
            line = line.split(";")[0] + "\n"
